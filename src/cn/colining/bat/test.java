...
public class test {
    ...
    {
       /*
       大根堆如果从一开始计数，ki就是k2i,k(2i+1)的父结点；
    ...
    {
        int temp = array[k];                            //当前要调整的父结点
       /*
        j是他的子结点，如果继续循环，说明当前结点已经向下移动了了一层
        */
        for (int j=2*k+1;j<length-1;j=2*k+1)
        {
            if (j<length&&array[j]<array[j+1])            //如果右结点比左结点大，就变化下标
            {
                j++;
            }
            if (temp>array[j])                                 //父结点大于当前子结点的最大值，说明这个父结点为首的大根堆没有问题
                break;
            else {
                array[k] = array[j];
                k = j;                    //记录父结点更换到的位置，继续下一次的循环
            }
        }
        array[k]= temp;                   //父结点可能会有多次移动，所以最后一次移动结束。再赋值
    }
